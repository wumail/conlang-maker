...
export const SCA_BOUNDARY_FEATURE = "__BOUNDARY__";
...
export function parseFeatureExpression(expr: string): FeatureExpression {
  ...
  for (const part of parts) {
    if (part.startsWith("+")) {
      const feat = part.slice(1);
      result.positive.push(feat === "#" ? SCA_BOUNDARY_FEATURE : feat);
    } else if (part.startsWith("-")) {
      const feat = part.slice(1);
      result.negative.push(feat === "#" ? SCA_BOUNDARY_FEATURE : feat);
    } else {
      // 无前缀默认为 positive
      result.positive.push(part === "#" ? SCA_BOUNDARY_FEATURE : part);
    }
  }
  return result;
}
...
export function findMatchingPhonemes(expr: FeatureExpression): string[] {
  ...
  const positive = expr.positive.filter((f) => f !== SCA_BOUNDARY_FEATURE);
  const negative = expr.negative.filter((f) => f !== SCA_BOUNDARY_FEATURE);
...
/** 对音素应用特征替换，返回结果音素 */
export function applyFeatureReplacement(
  phoneme: string,
...
function applyFeatureRule(
  ...
  const knownPhonemes =
    inventoryPhonemes.length > 0 ? inventoryPhonemes : getAllKnownPhonemes();
  const tokens = tokenizePhonemes(word, knownPhonemes);
  let changed = false;
  const resultTokens = [...tokens];
  const featureDetails: string[] = [];

  const hasBoundaryConstraint = (expr: FeatureExpression | null): boolean => {
    if (!expr) return false;
    return expr.positive.includes(SCA_BOUNDARY_FEATURE);
  };

  const hasAnyPhonemeFeatureConstraint = (
    expr: FeatureExpression | null,
  ): boolean => {
    if (!expr) return false;
    const pos = expr.positive.filter((f) => f !== SCA_BOUNDARY_FEATURE);
    const neg = expr.negative.filter((f) => f !== SCA_BOUNDARY_FEATURE);
    return pos.length > 0 || neg.length > 0;
  };

  const stripBoundaryConstraint = (
    expr: FeatureExpression,
  ): FeatureExpression => ({
    positive: expr.positive.filter((f) => f !== SCA_BOUNDARY_FEATURE),
    negative: expr.negative.filter((f) => f !== SCA_BOUNDARY_FEATURE),
  });
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (rule.context_before_features) {
      const beforeNeedsBoundary = hasBoundaryConstraint(
        rule.context_before_features,
      );
      if (beforeNeedsBoundary && i !== 0) continue;
      if (
        !beforeNeedsBoundary &&
        hasAnyPhonemeFeatureConstraint(rule.context_before_features)
      ) {
        if (i === 0) continue; // 无前文
        const beforeExpr = stripBoundaryConstraint(
          rule.context_before_features,
        );
        if (!matchesFeatures(tokens[i - 1], beforeExpr)) continue;
      }
    }

    // 检查 context_after
    if (rule.context_after_features) {
      const afterNeedsBoundary = hasBoundaryConstraint(
        rule.context_after_features,
      );
      if (afterNeedsBoundary && i !== tokens.length - 1) continue;
      if (
        !afterNeedsBoundary &&
        hasAnyPhonemeFeatureConstraint(rule.context_after_features)
      ) {
        if (i === tokens.length - 1) continue; // 无后文
        const afterExpr = stripBoundaryConstraint(rule.context_after_features);
        if (!matchesFeatures(tokens[i + 1], afterExpr)) continue;
      }
    }
    ...
    // 应用替换
    const newPhoneme = applyFeatureReplacement(
      tokens[i],
      rule.replacement_features,
    );
    if (newPhoneme !== tokens[i]) {
      const setStr = rule.replacement_features.set_features
        .map((f) => `+${f}`)
        .join(",");
      const rmStr = rule.replacement_features.remove_features
        .map((f) => `-${f}`)
        .join(",");
...
function normalizeBoundaryMarker(
  pattern: string,
  side: "before" | "after",
): string {
  ...
  if (!trimmed) return "";
  if (trimmed === "#") return "#";
  if (side === "before" && trimmed === "^") return "#";
  if (side === "after" && trimmed === "$") return "#";
  return trimmed;
}
...
function applySingleRule(
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    const beforeCtx = expandContextPattern(
      normalizeBoundaryMarker(rule.context_before, "before"),
      macros,
    );
    const afterCtx = expandContextPattern(
      normalizeBoundaryMarker(rule.context_after, "after"),
      macros,
    );
    ...
    try {
      ...
      console.warn(
        `SCA: Invalid regex pattern "${pattern}" for rule "${rule.rule_id}"`,
      );
    }
  }
...
export function applySoundChanges(
  ...
  for (const ruleSet of sorted) {
    for (const rule of ruleSet.rules) {
      ...
      const { result, changed, featureDetails } = applySingleRule(
        current,
        rule,
        macros,
        inventoryPhonemes,
