...
可在侧边栏「高级搜索」中按状态过滤，也可在词条编辑面板的 IPA 下方查看同样标签。

:::tip
若词条已标记为「废弃」，会优先显示废弃状态，不再同时显示「变化」标签。
:::
...
对于来源为 `演化 (Evolved)` 或 `借词 (Borrowed)` 且存在父词引用的词条，编辑器会显示演化路径。

- 支持跨多代语言显示全路径（例如：`a → ab → abc`）
- 相邻重复词形会自动去重（例如：`a → a` 显示为 `a`）
- 下方保留已应用音变规则标签（若存在）
